...
这是一个面向 Qwen3-4B Instruct 的 LoRA 监督微调工程。代码按训练链路拆分为
数据预处理、tokenizer、模型加载、LoRA 注入、dataset/collator、Trainer、推理和
权重合并几部分；拆分只服务于可读性和测试，不做过度封装。
...
5. 加载 adapter 推理：见 [docs/04_inference_merge.md](docs/04_inference_merge.md)
6. 按需合并完整模型：见 [docs/04_inference_merge.md](docs/04_inference_merge.md)
7. 运行本地案例测试：见 [docs/05_testing.md](docs/05_testing.md)
...
- 环境由 uv 管理，Python 版本固定为 `.python-version` 中的 `3.11`。
- DeepSpeed 是可选依赖，需要时执行 `uv sync --extra deepspeed`。
- 单卡 LoRA 起步建议先使用 `max_length=1024` 或 `2048`，batch size 1。
- 训练前先用小数据跑通预处理、训练、推理闭环，再扩大数据规模。
- `tests/` 中的预处理案例不依赖 torch；训练和推理辅助测试会在缺少深度学习依赖时自动跳过。
