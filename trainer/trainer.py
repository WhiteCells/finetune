...
def validate_train_config(config: TrainConfig) -> None:
    ...
    if not config.train_file.strip():
        raise ValueError("`train_file` 不能为空。")
    ...
    if config.evaluation_strategy == "steps" and config.eval_steps <= 0:
        raise ValueError("`eval_steps` 必须大于 0。")
    if config.save_total_limit is not None and config.save_total_limit <= 0:
        raise ValueError("`save_total_limit` 必须大于 0。")
